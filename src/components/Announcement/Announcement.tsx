...
const Announcement: React.FC<IAnnouncementProps> = ({ advertisments }) => {
  ...
  const [modalData, setModalData] = React.useState<{ title: string; body: string }>({ title: '', body: '' })
  ...
  const handleOpenAddModal = (data: string) => {
    ...
    setModalData({ title: data, body: String(modalBody?.attributes.body) || '' })
    setOpenModal(true)
  }
        ...
        <div className={styles['container']}>
          <div className={styles['announcement__inner']}>
            ...
            <div className={styles['announcement__content']}>
              ...
              <div className={styles['announcement__slider-item']} onClick={() => handleOpenAddModal(modalData.title)}>
                {modalData.title}
              </div>
