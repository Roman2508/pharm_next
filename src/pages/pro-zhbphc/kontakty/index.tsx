...
import { GetAdministrationQuery, GetHeaderQuery, GetMainScreenQuery } from '@/graphql/__generated__'
...
const Contacts: NextPage<IContactsPageProps> = ({ headerData, mainScreenData, administration }) => {
  const [messageTopic, setMessageTopic] = React.useState(topicList[0].text)
  ...
  React.useEffect(() => {
    if (topicList[0].text === messageTopic) {
      setTopicError(true)
    } else {
      setTopicError(false)
    }
  }, [messageTopic, isSubmitting])
              ...
              <form className={styles['form']} onSubmit={handleSubmit(onSubmit)}>
                ...
                <Input
                  {...register('email', {
                    ...
                    pattern: { value: validEmail, message: 'Не вірний формат пошти' },
                  })}
                  width="100%"
                  label="Ваш E-mail"
                ...
                <Textarea
                  {...register('body', { required: "Текст повідомлення обов'язковий" })}
                  width="100%"
