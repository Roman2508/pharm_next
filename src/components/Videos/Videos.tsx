...
export const Videos: React.FC<IVideoProps> = ({ test, videos }) => {
  ...
  return (
    <>
      <FullScreenFrame
        setOpenFullScreen={setOpenFullScreenVideo}
        isOpenFullScreen={openFullScreenVideo}
      >
        <iframe
          id="ytplayer"
          width="60%"
      ...
      >
        ...
        >
          {videos.videos.data.map((video, index) => (
            <div
              key={video.id}
              className={cn(styles["slide-wrapper"], "keen-slider__slide")}
            >
              <div
              ...
              >
                <h3 className={styles["videos__slider-title"]}>
